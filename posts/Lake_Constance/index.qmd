...
![With ChatGPT generatet header image](images/ramp_image_Chat_GPT.png){#fig-header}
...
- \(\mathbf{x}_t\) the feature vector observed on day \(t\).

The one-step-ahead forecasting problem is to construct a model \(f(\cdot)\) such that

\[
\hat{y}_{t+1} = f(\mathbf{x}_t), \qquad t = 1,\dots,T-1,
\]
...
\[
y_{t+1} = f(\mathbf{x}_t) + \varepsilon_{t+1},
\]

where \(\mathbf{x}_t\) contains contemporaneous and lagged features and \(\varepsilon_{t+1}\) is an error term with mean zero.

### Data {#sec-data}

We combine several datasets to construct \(\mathbf{x}_t\):
...
so that each row \((\mathbf{x}_t, \tilde{y}_t)\) encodes features at day \(t\) and the corresponding water level on day \(t+1\).
...
\[
\tilde{z}_t = \beta_0 + \mathbf{x}_t^\top \boldsymbol{\beta} + \varepsilon_t,
\]
...
\[
\hat{z}_{t+1}^{\text{RF}} = \frac{1}{B} \sum_{b=1}^{B} f^{(b)}(\mathbf{x}_t).
\]
